...
export async function deployFromSource(opts: Options): Promise<void> {
  ...
  try {
    ...
    const startRes = await fetch(
      `${apiBase}/v1/${appId}/frontend/deployments/from-source/${created.deployment_id}/start`,
      {
        method: 'POST',
        headers: {
          authorization: `Bearer ${apiKey}`,
    ...
    // 6. Tail SSE logs
    await streamSSE(
      `${apiBase}/v1/${appId}/frontend/deployments/from-source/${created.deployment_id}/logs`,
      { authorization: `Bearer ${apiKey}` },
    );
