...
export class SupportTicketDO extends DurableObject {
  ...
  async fetch(req) {
    ...
    // ctx.invokeDO arrives at:
    //   https://internal.butterbase/_dispatch/{class}/{instance}
    //   with x-butterbase-internal-caller = "fn:<name>" | "do:<name>:<key>"
    //        x-butterbase-instance         = instance key (= ticket_id here)
    //        x-butterbase-class            = class name
    // Public HTTP arrives at:
    //   https://{subdomain}.butterbase.dev/_do/{class-kebab}/{ticket_id}
    ...
    // Public path is /_do/{class}/{ticket_id} — parts[2]. Invoker path is
    // /_dispatch/{class}/{instance} — parts[2] also — either way it's index 2.
    ...
    // Topological auth: presence of x-butterbase-internal-caller proves the
    // request came through the platform's do-invoker Worker (only the
    // dispatcher can set that header — public HTTP is stripped). Bearer check
    // remains as a fallback for direct-HTTP callers using INTERNAL_FN_KEY.
    let userId;
    if (invokerCaller) {
      userId = 'internal';
    } else {
      ...
      const internalKey = this.env.BUTTERBASE_INTERNAL_FN_KEY;
      if (internalKey && token === internalKey) userId = 'internal';
      else userId = await this.verifyTeamMembership(token);
    }
  ...
  apiHeaders() {
    // Must be BUTTERBASE_API_KEY (bb_sk_ app service key), not INTERNAL_FN_KEY.
    // INTERNAL_FN_KEY is authenticated but NOT service-role — auto-REST auto-CRUD
    // enforces RLS against it and every SELECT/INSERT/PATCH the DO makes gets
    // filtered or 403'd (AUTH_RLS_POLICY_VIOLATION). Verified 2026-07-14 by
    // instrumenting the deployed DO with a DO-storage trace: swapping the
    // bearer back to BUTTERBASE_API_KEY restored the full agent loop.
    // AI gateway (/chat/completions, /rag, /embeddings) rejects bb_sk_ too —
    // those routes are invoked through the ai-chat-completion / ai-rag-query
    // wrapper functions, which in turn auth with their own AI_GATEWAY_KEY.
  ...
  // Propose a substrate capability via the INTERNAL_TOKEN-guarded
  // do-substrate-propose function. The DO has no ctx.substrate, so it goes
  // over HTTP. Returns the substrate verdict (action_id, requires_approval, result).
  ...
  // Read from substrate via the INTERNAL_TOKEN/service_key-guarded substrate-read-internal
  // function. Same HTTP transport pattern as proposeToSubstrate — the DO has no ctx.substrate.
  // Re-projects the latest ticket state (status, messages, diagnoses, escalations)
  // as a substrate source_artifact. Idempotent upsert keyed by ticket id, safe to
  // re-fire after every state change. Fire-and-forget — never blocks the agent loop.
  async syncTicketArtifact() {
    if (!this.ticketId) return;
    try {
      const url = `${this.env.BUTTERBASE_API_URL}/v1/${this.env.BUTTERBASE_APP_ID}/fn/sync-ticket-artifact`;
      const r = await fetch(url, {
        method: 'POST',
        headers: this.apiHeaders(),
        body: JSON.stringify({ ticket_id: this.ticketId }),
      });
      if (!r.ok) {
        console.warn('[SupportTicketDO] sync-ticket-artifact non-2xx', r.status, (await r.text().catch(() => '')).slice(0, 200));
      }
    } catch (err) {
      console.warn('[SupportTicketDO] sync-ticket-artifact threw', err?.message);
    }
  }
  ...
  async runAgentLoop(mode, hint) {
    ...
    if (mode !== 'escalate') {
      ...
      // Stash on instance so toolPropose_draft_reply can pick the role/status
      // without re-querying autonomy_settings.
    ...
    while (!done && iter < MAX_LLM_CALLS) {
      ...
      try {
        ...
        // Chat completions rejects BUTTERBASE_INTERNAL_FN_KEY; go through the
        // ai-chat-completion wrapper function.
        const r = await fetch(`${this.env.BUTTERBASE_API_URL}/v1/${this.env.BUTTERBASE_APP_ID}/fn/ai-chat-completion`, {
    ...
    this.state.waitUntil(this.syncTicketArtifact());
  }

  async toolSearch_docs(args) {
    // RAG routes reject BUTTERBASE_INTERNAL_FN_KEY; go through the ai-rag-query
    // wrapper function which auths with its own auto-minted BUTTERBASE_API_KEY.
    const url = `${this.env.BUTTERBASE_API_URL}/v1/${this.env.BUTTERBASE_APP_ID}/fn/ai-rag-query`;
    const r = await fetch(url, {
      method: 'POST', headers: this.apiHeaders(),
      body: JSON.stringify({
        collection: this.env.RAG_COLLECTION,
        query: args.query,
  ...
  async toolPropose_diagnosis(args) {
    ...
    // Also project the diagnosis as a substrate source_artifact so another
    // agent (CRM copilot, founder's Claude) can re-read it. Best-effort —
    // local diagnosis row is the source of truth; substrate is a paper trail.
  ...
  async toolPropose_action(args) {
    ...
    // Mirror as agent_proposals so the existing approve-proposal /
    // reject-proposal functions and the founder UI can act on it.
  ...
  async toolPropose_draft_reply(args) {
    ...
    // Mirror auto-reply-worker.ts:557+ so the DO honors autonomy_settings.
    // draft_for_approval (default): role='agent_draft', status='awaiting_approval' — founder must send.
    // auto_send:    role='founder', status='open',     reply is sent immediately.
    // auto_resolve: role='founder', status='resolved', reply sent and ticket closed.
    ...
    // For auto-sent replies, push immediately to the widget DO so the
    // customer sees it without a console-driven send-draft-reply round trip.
    if (role === 'founder') {
  ...
  // Mirrors customerEscalationCopy() in auto-reply-worker so a customer sees
  // identical handoff text regardless of which path (DO vs worker) escalated.
  ...
  async pushToWidgetDo(frame) {
    // Native Cloudflare DO service binding (baked into DO Workers at deploy).
    // Replaces the prior HTTP + INTERNAL_TOKEN pattern — no URL, no bearer,
    // no cross-clone attack surface. Auth is topological (same-app binding).
    const ns = this.env.WIDGET_TICKET_DO;
    if (!ns) {
      console.warn('[SupportTicketDO] WIDGET_TICKET_DO binding missing — skip push');
      return;
    }
    try {
      const stub = ns.get(ns.idFromName(this.ticketId));
      const r = await stub.fetch(`https://do-internal/_do/widget-ticket-do/${this.ticketId}?cmd=push`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ cmd: 'push', ticket_id: this.ticketId, ...frame }),
      });
      if (!r.ok) {
  ...
  async toolPropose_escalation(args) {
    ...
    // Always send a customer-visible handoff so they're not left hanging.
    // Mirrors auto-reply-worker.dispatchEscalation: role='founder' (auto-sent,
    // no approval needed), pushed to the widget DO immediately. The full
    // escalation (email/Slack) still fires via execute-escalation below.
    ...
    // First: record the escalation on the substrate ledger as a source_artifact
    // so other agents (CRM, founder copilot) can re-read it. The actual delivery
    // (Slack/email via execute-escalation) still goes through the HMAC outbox
    // below. If substrate is unreachable we fall back to a self-minted id so
    // escalation delivery never blocks (always-escalate floor).
    let actionId = null;
    try {
      const verdict = await this.proposeToSubstrate('upsert_source_artifact', {
  ...
  async classifyIssueType() {
    ...
    let resp;
    try {
      // Same wrapper-fn indirection as runAgentLoop's chat call.
      const r = await fetch(`${this.env.BUTTERBASE_API_URL}/v1/${this.env.BUTTERBASE_APP_ID}/fn/ai-chat-completion`, {
        method: 'POST', headers: this.apiHeaders(),
        body: JSON.stringify({
          model: this.env.HAIKU_MODEL || 'anthropic/claude-haiku-4.5',
          max_tokens: 60,
          messages: [
