...
export default async function handler(req, ctx) {
  ...
  if (!(await ctx.idempotency.claim(proposal_id, { scope: "reject_proposal", ttlSeconds: 3600 }))) {
  ...
  try {
    if (p.substrate_action_id) {
      substrateResult = await substrateDecide(ctx.env, p.substrate_action_id, 'reject', reason);
    }
  } catch (err) {
    console.error("reject-proposal: substrate reject failed", err?.message);
    return json({ error: "substrate_reject_failed", message: err?.message }, 502);
  }
  ...
  await ctx.db.query(
    `INSERT INTO activities (actor_user_id, kind, entity_type, entity_id, payload)
     VALUES ($1, 'proposal.rejected', 'agent_proposal', $2::text, $3)`,
    [ctx.user.id, proposal_id, JSON.stringify({ ticket_id: p.ticket_id, capability: p.capability, reason: reason || null })]
  );
