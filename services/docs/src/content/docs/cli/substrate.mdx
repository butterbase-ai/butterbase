...
|---|---|---|
| `--kinds <k,...>` | comma-separated: `decisions`, `commitments`, `learnings`, `source_artifacts` | _all_ |
| `--source-artifact-id <id>` | string — restrict to rows linked to this source artifact (source_artifact rows excluded when set) | _none_ |
| `--superseded <bool>` | `true` \| `false` — when `false`, excludes superseded decisions and expired commitments | _include all_ |
| `--before <timestamp>` | ISO timestamp keyset cursor (paste `next_before` from a previous response) | _none_ |
