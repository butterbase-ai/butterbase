...
function makeRuntimePool({
  ...
  claimRows = [{ status: 'resolved' }] as { status: string }[],
  failClaim = false,
} = {}) {
...
describe('POST /v1/webhooks/enrichlayer/email', () => {
  ...
  it('1. valid nonce + email present → resolved, credits deducted, audit written, 200 { ok: true }', async () => {
    const pool = makeRuntimePool({ claimRows: [{ status: 'resolved' }] });
  ...
  it('2. valid nonce + null email → failed row, $0 charge, no deductCreditsBalance', async () => {
    const pool = makeRuntimePool({ claimRows: [{ status: 'failed' }] });
    vi.mocked(runtimePoolFor).mockReturnValue(pool as any);
    ...
    // Audit row written with usd_cost=0 and usd_charged=0
    const auditCall = pool.query.mock.calls.find(
      (c: unknown[]) =>
        typeof c[0] === 'string' && (c[0] as string).includes('enrichlayer_usage_logs'),
    );
  ...
  it('5. concurrent same-nonce: second call claims 0 rows → ignored, no double charge', async () => {
    ...
    const queryFn = vi.fn().mockImplementation((sql: string) => {
      ...
      if (sql.startsWith('UPDATE')) {
        ...
        const rows = callCount === 1 ? [{ status: 'resolved' }] : [];
