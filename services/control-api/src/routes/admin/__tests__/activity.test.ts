...
// Timeseries test fixtures
let tsUser1Id: string;
let tsUser1OrgId: string;
let tsUser2Id: string;
let tsUser2OrgId: string;
let tsVisitAppId1: string;
let tsVisitAppId2: string;
...
beforeAll(async () => {
  ...
  // ── Timeseries: two extra platform users with activity on days -15/-16/-17 ──
  tsUser1Id = randomUUID();
  tsUser2Id = randomUUID();

  const tsOrg1Res = await pool.query<{ id: string }>(
    `INSERT INTO organizations
       (owner_id, name, personal, plan_id, credits_usd, auto_refill_enabled, account_status)
     VALUES ($1, 'test-timeseries-org1', true, 'playground', 0, false, 'active')
     RETURNING id`,
    [tsUser1Id],
  );
  tsUser1OrgId = tsOrg1Res.rows[0].id;

  const tsOrg2Res = await pool.query<{ id: string }>(
    `INSERT INTO organizations
       (owner_id, name, personal, plan_id, credits_usd, auto_refill_enabled, account_status)
     VALUES ($1, 'test-timeseries-org2', true, 'playground', 0, false, 'active')
     RETURNING id`,
    [tsUser2Id],
  );
  tsUser2OrgId = tsOrg2Res.rows[0].id;

  await pool.query(
    `INSERT INTO platform_users
       (id, email, personal_organization_id, last_login_at, last_activity_at)
     VALUES
       ($1, 'test-timeseries-user1@example.com', $2, now(), now()),
       ($3, 'test-timeseries-user2@example.com', $4, now(), now())`,
    [tsUser1Id, tsUser1OrgId, tsUser2Id, tsUser2OrgId],
  );

  await pool.query(
    `INSERT INTO platform_user_activity_daily (user_id, day, action_count) VALUES
     ($1, CURRENT_DATE - 15, 4), ($1, CURRENT_DATE - 16, 2), ($1, CURRENT_DATE - 17, 6),
     ($2, CURRENT_DATE - 15, 3), ($2, CURRENT_DATE - 16, 1), ($2, CURRENT_DATE - 17, 5)
     ON CONFLICT (user_id, day) DO UPDATE SET action_count = EXCLUDED.action_count`,
    [tsUser1Id, tsUser2Id],
  );

  // ── Timeseries: frontend-visit rows on days -25 and -26 ──
  tsVisitAppId1 = randomUUID();
  tsVisitAppId2 = randomUUID();

  await pool.query(
    `INSERT INTO frontend_visit_daily (app_id, day, request_count, unique_visitor_count) VALUES
     ($1, CURRENT_DATE - 25, 10, 3),
     ($2, CURRENT_DATE - 25, 20, 5),
     ($1, CURRENT_DATE - 26, 7,  2)
...
afterAll(async () => {
  ...
  if (tsVisitAppId1 || tsVisitAppId2) {
    for (const appId of [tsVisitAppId1, tsVisitAppId2].filter(Boolean)) {
      await pool.query(`DELETE FROM frontend_visit_daily WHERE app_id = $1`, [appId]);
    }
  }
  if (tsUser1Id || tsUser2Id) {
    for (const uid of [tsUser1Id, tsUser2Id].filter(Boolean)) {
      await pool.query(`DELETE FROM platform_user_activity_daily WHERE user_id = $1`, [uid]);
    }
    for (const uid of [tsUser1Id, tsUser2Id].filter(Boolean)) {
      await pool.query(`DELETE FROM platform_users WHERE id = $1`, [uid]);
    }
    if (tsUser1OrgId) await pool.query(`DELETE FROM organizations WHERE id = $1`, [tsUser1OrgId]);
    if (tsUser2OrgId) await pool.query(`DELETE FROM organizations WHERE id = $1`, [tsUser2OrgId]);
  }
  if (testUserId) {
...
// ─── 6. Timeseries: active-users ────────────────────────────────────────────
describe('GET /admin/activity/timeseries/active-users', () => {
  it('returns rows sorted ASC with correct distinct user counts for seeded days', async () => {
    // Use days=20 so CURRENT_DATE-15 through -17 fall within the window.
    const r = await app.inject({ method: 'GET', url: '/admin/activity/timeseries/active-users?days=20' });
    expect(r.statusCode).toBe(200);
    const body: Array<{ day: string; count: number }> = r.json();
    expect(Array.isArray(body)).toBe(true);

    // Rows must be sorted ASC.
    for (let i = 1; i < body.length; i++) {
      expect(body[i].day >= body[i - 1].day).toBe(true);
    }

    // Our 3 seeded days should each have at least 2 distinct users.
    ...
    for (const offset of [15, 16, 17]) {
      const day = fmtDay(offset);
      const row = body.find(r => r.day === day);
      expect(row, `expected row for day ${day}`).toBeDefined();
      expect(row!.count).toBeGreaterThanOrEqual(2);
    }
  });
});

// ─── 7. Timeseries: deploys ──────────────────────────────────────────────────
describe('GET /admin/activity/timeseries/deploys', () => {
  it('merges counts from multiple regions and returns sorted ASC', async () => {
    mockFanOutQuery.mockResolvedValueOnce([
      { day: '2026-07-01', count: 2 },
      { day: '2026-07-01', count: 3 },
      { day: '2026-07-02', count: 1 },
    ] as any);

    const r = await app.inject({ method: 'GET', url: '/admin/activity/timeseries/deploys' });
    expect(r.statusCode).toBe(200);
    const body: Array<{ day: string; count: number }> = r.json();
    expect(body).toEqual([
...
// ─── 8. Timeseries: frontend-visits ─────────────────────────────────────────
describe('GET /admin/activity/timeseries/frontend-visits', () => {
  it('returns 2 rows with summed request_count and unique_visitor_count for seeded days', async () => {
    // days=28 covers CURRENT_DATE-27 through today — includes our days -25 and -26.
    const r = await app.inject({ method: 'GET', url: '/admin/activity/timeseries/frontend-visits?days=28' });
    expect(r.statusCode).toBe(200);
    const body: Array<{ day: string; request_count: number; unique_visitor_count: number }> = r.json();
    expect(Array.isArray(body)).toBe(true);
    ...
    // day -25: two apps → request_count >= 30 (10+20), unique_visitor_count >= 8 (3+5)
    const day25 = fmtDay(25);
    const row25 = body.find(r => r.day === day25);
    expect(row25, `expected row for day ${day25}`).toBeDefined();
    expect(row25!.request_count).toBeGreaterThanOrEqual(30);
    expect(row25!.unique_visitor_count).toBeGreaterThanOrEqual(8);

    // day -26: one app → request_count >= 7, unique_visitor_count >= 2
    const day26 = fmtDay(26);
    const row26 = body.find(r => r.day === day26);
    expect(row26, `expected row for day ${day26}`).toBeDefined();
    expect(row26!.request_count).toBeGreaterThanOrEqual(7);
    expect(row26!.unique_visitor_count).toBeGreaterThanOrEqual(2);
  });
});

// ─── 9. days=91 is capped at 90 ─────────────────────────────────────────────
describe('days param cap', () => {
  it('days=91 is capped to 90 — endpoint responds 200 without error', async () => {
    const r = await app.inject({ method: 'GET', url: '/admin/activity/timeseries/active-users?days=91' });
    expect(r.statusCode).toBe(200);
    expect(Array.isArray(r.json())).toBe(true);
  });
});

// ─── 10. Recent activity: limit param ───────────────────────────────────────
