// Frontend (static) "from source" deployment routes
...
const startSchema = z.object({
  ...
  outputDir: z.string().min(1).max(200).default('dist'),
...
export async function registerFrontendFromSourceRoutes(fastify: FastifyInstance) {
  ...
  // POST /v1/:appId/frontend/deployments/from-source
  ...
  fastify.post('/v1/:appId/frontend/deployments/from-source', async (request, reply) => {
    ...
    await controlDb.query(
      `INSERT INTO app_build_jobs
       ...
       VALUES ($1, $2, 'frontend', 'PENDING', $3, $4)`,
      [
        buildId,
    ...
    logFromRequest(request, {
      ...
      eventData: { source: 'from-source', deploy_type: 'frontend' },
      success: true,
    });
  ...
  // POST /v1/:appId/frontend/deployments/from-source/:deploymentId/start
  ...
  fastify.post('/v1/:appId/frontend/deployments/from-source/:deploymentId/start', async (request, reply) => {
    ...
    const job = await controlDb.query(
      `SELECT id, deployment_id FROM app_build_jobs
        WHERE deployment_id = $1 AND deploy_type = 'frontend' AND status = 'PENDING'
    ...
    return reply.code(202).send({
      ...
      status: 'BUILDING',
      logs_url: `/v1/${appId}/frontend/deployments/from-source/${deploymentId}/logs`,
      status_url: `/v1/${appId}/frontend/deployments/${deploymentId}`,
    });
  });

  // GET /v1/:appId/frontend/deployments/from-source/:deploymentId/logs
  ...
  fastify.get('/v1/:appId/frontend/deployments/from-source/:deploymentId/logs', async (request, reply) => {
    ...
    const job = await controlDb.query(
      `SELECT id, status, log_r2_key FROM app_build_jobs
        WHERE deployment_id = $1 AND deploy_type = 'frontend'
        ORDER BY created_at DESC LIMIT 1`,
      [deploymentId],
    );
    if ((job.rowCount ?? 0) === 0) {
